...
Stage 8-Q | Combat Terrain — Obstacles and Bonus Squares
Status: Stage 8-P complete. Active hunter selection and deployment phase
both working. Full hunter movement UI verified.
...
**Resuming from:** Stage 8-P complete — active hunter selection working; hunter deployment phase and full movement UI verified
...
**Commit:** `"8Q: Combat terrain — obstacle cells and bonus terrain squares"`
**Next session:** STAGE_08_R.md
...
/* Sticky mud — accuracy penalty (negative accuracyBonus) */
.grid-cell--terrain-mud {
    background-color: rgba(55, 45, 20, 0.40);
    border-color:     rgba(80, 65, 30, 0.65);
    border-width:     1px;
}
```
...
After implementation, create three mock `TerrainCellSO` assets for testing:
- `Assets/_Game/Data/Terrain/Terrain_HighGround.asset` — accuracyBonus=+1, cssClass="grid-cell--terrain-high"
  *(Hunter standing on elevated rock or large object — easier to hit the monster from above)*
- `Assets/_Game/Data/Terrain/Terrain_BoneAsh.asset` — defenseBonus=+1, cssClass="grid-cell--terrain-ash"
  *(Soft ground absorbs impacts — incoming Force roll reduced)*
- `Assets/_Game/Data/Terrain/Terrain_StickyMud.asset` — accuracyBonus=-1, cssClass="grid-cell--terrain-mud"
  *(Hunter stuck in mud — accuracy penalty on to-hit rolls)*

And one `TerrainSetupSO`:
- `Assets/_Game/Data/Terrain/TerrainSetup_GauntArena.asset`
  - HighGround at (5,4) and (16,4)
  - BoneAsh at (11,8)
  - StickyMud at (8,12) and (14,12)
...
- [ ] Hunter standing on sticky mud gets -1 accuracy on Precision Check — confirm via d10 log
- [ ] Hunter standing on ash terrain reduces incoming Force Check by defenseBonus — confirm via d10 log
...
**File:** `_Docs/Stage_08/STAGE_08_R.md`
**Covers:** Combat Loop Integration Gate — full Aldric vs Gaunt Standard pass through the new health model (behavior deck as life, wound location draws, defeat condition), hunter deployment, and terrain; no new features, bugs only
